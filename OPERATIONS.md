...
> ⚠️ **SSOT 절대 원칙 (2026-05-18 신설)** — 사이트의 모든 데이터는 `_build/` JSON (admin DB)을 기준으로 한다. HTML/JS의 변수·상수에 운영 가능한 데이터를 박지 말 것. 박혀 있는 것이 발견되면 마이그레이션 대상. 자세히는 `CRITICAL_RULES.md § 7` 및 `STRATEGY.md § 6.0` 참조.
